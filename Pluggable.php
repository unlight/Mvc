...
abstract class Pluggable {
	...
	public function addMutator($mutatorClass) {
		for ($mutator = $this->mutator; $mutator; $mutator = $mutator->nextMutator) {
			if (get_class($mutator) == $mutatorClass) return;
		}

		$mutator = new $mutatorClass($this);
		if ($this->mutator === null) {
			$this->mutator = $mutator;
		} else {
			$this->mutator->addMutator($mutator);
		}
	}
}